...
<head>
    ...
    <title>FIT PRO | RÉCUPÉRATION D'ACCÈS</title>
    ...
    <style>
        ...
        .mesh-bg { position: fixed; inset: 0; z-index: -2; background-image: radial-gradient(at 100% 100%, rgba(255, 237, 0, 0.08) 0px, transparent 50%), linear-gradient(rgba(255, 255, 255, 0.02) 1px, transparent 1px), linear-gradient(90deg, rgba(255, 255, 255, 0.02) 1px, transparent 1px); background-size: 100% 100%, 40px 40px, 40px 40px; }
...
<body>
    ...
    <div class="login-terminal">
        <div class="terminal-header">
            ...
            <div style="font-size: 9px; color: var(--accent); letter-spacing: 2px; font-weight: 800; margin-top: 5px;">SIGNAL_RÉCUPÉRATION: PRÊT</div>
        </div>
        <div class="p-4 p-lg-5">
            <div class="text-center mb-5">
                <div class="user-avatar-hex">
                    <i class="fas fa-satellite-dish text-accent" style="z-index: 2; font-size: 20px;"></i>
                    <svg class="hex-svg" viewBox="0 0 100 100"><polygon points="50 1, 95 25, 95 75, 50 99, 5 75, 5 25" /></svg>
                </div>
                <h5 class="fw-900 uppercase m-0">Réinitialiser l'Accès</h5>
                <p class="text-secondary small mt-1">Recevez un lien de réinitialisation par e-mail</p>
            </div>

            @if (session('status'))
                <div class="alert bg-success bg-opacity-10 text-success border-0 rounded-4 text-center mb-4 small fw-bold">
                    {{ session('status') }}
                </div>
            @endif
            ...
            <form method="POST" action="{{ route('password.email') }}">
                ...
                <div class="mb-4">
                    ...
                    <input type="email" name="email" class="form-control-tactical w-100 @error('email') is-invalid @enderror" value="{{ old('email') }}" required autofocus placeholder="contact@fitpro.com">
                    @error('email') <div class="text-danger small mt-2 fw-bold">{{ $message }}</div> @enderror
                </div>
                ...
                <div class="text-center mt-4">
                    <a href="{{ route('login') }}" style="color: rgba(255,255,255,0.3); font-size: 10px; text-decoration: none; text-transform: uppercase; font-weight: 800;">
