...
@section('page_title', 'MODIFIER UN COACH')

@section('content')
<div class="container-fluid py-4 px-lg-5">

    <!-- 1. En-tête (Header Elite) -->
    <div class="d-flex justify-content-between align-items-center mb-5">
        <div>
            <h1 class="fw-900 text-white mb-1 uppercase letter-spacing-lg">MODIFIER <span style="color: var(--accent);">LE PROFIL</span></h1>
            <p class="text-secondary small uppercase letter-spacing-sm" style="font-size: 10px;">MODIFICATION DES PROTOCOLES DU COACH #{{ 1000 + $coach->id }}</p>
        </div>
        <a href="{{ route('admin.coachs.index') }}" class="elite-btn-back uppercase fw-900 letter-spacing-xs">
            ...
            <div class="edit-terminal">
                ...
                <div class="terminal-top-bar">
                    ...
                    <div class="protocol-status uppercase fw-900 letter-spacing-xs">
                        <span class="status-dot"></span> PROFIL_COACH : ÉDITION_PROTOCOLE
                    </div>
                    <div class="sys-id fw-900 letter-spacing-xs">SYS_ID // {{ 1000 + $coach->id }}</div>
                </div>
                ...
                <div class="coach-identity-bar">
                    <div class="avatar-hex">
                        ...
                        @else
                            <span class="avatar-letter fw-900">{{ strtoupper(substr($coach->user->name, 0, 1)) }}</span>
                        @endif
                        <svg class="hex-svg" viewBox="0 0 100 100"><polygon points="50 1, 95 25, 95 75, 50 99, 5 75, 5 25" /></svg>
                    </div>
                    <div class="ms-4">
                        <h4 class="m-0 fw-900 text-white uppercase letter-spacing-sm">{{ $coach->user->name }}</h4>
                        <span class="text-accent uppercase fw-900 letter-spacing-xs" style="font-size: 10px;">
                            SPÉCIALISATION : {{ strtoupper($coach->typeSeance->nom ?? 'Standard') }}
                        </span>
                    </div>
                </div>
                ...
                <div class="p-4 p-lg-5">
                    <form action="{{ route('admin.coachs.update', $coach->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row g-4">
                            ...
                            <div class="col-md-4">
                                ...
                                <div class="neo-input-group mt-2">
                                    ...
                                    <input type="text" name="name" value="{{ $coach->user->name }}" required placeholder="NOM DU COACH..." class="uppercase fw-900 letter-spacing-xs">
                                </div>
                            </div>
                            ...
                            <div class="col-md-4">
                                ...
                                <div class="neo-input-group mt-2">
                                    ...
                                    <input type="email" name="email" value="{{ $coach->user->email }}" required placeholder="EMAIL@SYSTEM.COM..." class="uppercase fw-900 letter-spacing-xs">
                                </div>
                            </div>
                            ...
                            <div class="col-md-4">
                                ...
                                <div class="neo-input-group mt-2">
                                    ...
                                    <input type="text" name="telephone" value="{{ $coach->telephone }}" required placeholder="+212 6XX..." class="uppercase fw-900 letter-spacing-xs">
                                </div>
                            </div>

                            <!-- Spécialisation Selection -->
                            <div class="col-md-12">
                                <label class="tiny-caps letter-spacing-sm">DISCIPLINE PRINCIPALE ASSIGNÉE</label>
                                <div class="neo-input-group mt-2">
                                    <i class="fas fa-dumbbell"></i>
                                    <select name="type_seance_id" required class="uppercase fw-900 letter-spacing-xs">
                                        @foreach($categories as $cat)
                                            <option value="{{ $cat->id }}" {{ $coach->type_seance_id == $cat->id ? 'selected' : '' }}
...
<style>
    ...
    .avatar-letter { color: white; z-index: 2; font-size: 1.5rem; }
