...
@section('page_title', 'CENTRE DE COMMANDEMENT DU CLIENT')

@section('content')
<div class="container-fluid py-4 px-lg-5">
    
    <div class="row g-4 justify-content-center">
        <!-- 1. Left Column: Tactical Adhérent Card -->
        <div class="col-lg-4">
            <div class="profile-card-tactical shadow-lg">
                <div class="card-header-line"></div>
                
                <div class="text-center p-5">
                    <!-- Hexagon Profile Picture (Symétrie Coach Design) -->
                    <div class="user-avatar-hex mx-auto mb-4" style="width: 150px; height: 150px;">
                        @if($user->image)
                            <div class="hex-image-wrap">
                                <img src="{{ asset('storage/' . $user->image) }}" class="hex-img" alt="Adhérent">
                            </div>
                        @else
                        ...
                        <!-- Status Indicator Pip (Green if active, Red if expired) -->
                        <div class="admin-crown-pip {{ $user->expired_at && \Carbon\Carbon::now()->lt($user->expired_at) ? 'bg-success-pip' : 'bg-danger-pip' }}">
                            <i class="fas fa-shield-alt"></i>
                        </div>
                    </div>

                    <h2 class="fw-900 text-white mb-1 uppercase letter-spacing-sm">{{ $user->name }}</h2>
                    <div class="badge-clearance mb-4 uppercase fw-900 letter-spacing-xs">NIVEAU 01 • ADHÉRENT ÉLITE</div>

                    <!-- Quick Info List -->
                    <div class="tactical-data-list text-start">
                        <div class="data-item">
                            <span class="label uppercase fw-700">Identifiant Membre</span>
                            <span class="value fw-900 text-white">#ADH-{{ 1000 + $user->id }}</span>
                        </div>
                        <div class="data-item">
                            <span class="label uppercase fw-700">Pack Actif</span>
                            <span class="value fw-900 text-accent">{{ strtoupper($user->typeAbonnement->nom ?? 'AUCUN') }}</span>
                        </div>
                        <div class="data-item">
                            <span class="label uppercase fw-700">Expiration</span>
                            <span class="value fw-900 text-white">{{ $user->expired_at ? \Carbon\Carbon::parse($user->expired_at)->format('d/m/Y') : 'N/A' }}</span>
                        </div>
                        <div class="data-item border-0">
                            <span class="label uppercase fw-700">Statut Signal</span>
                            <span class="value fw-900 uppercase {{ $user->expired_at && \Carbon\Carbon::now()->lt($user->expired_at) ? 'text-success' : 'text-danger' }}">
                                {{ $user->expired_at && \Carbon\Carbon::now()->lt($user->expired_at) ? 'Opérationnel' : 'Expiré' }}
                            </span>
                        </div>
                    </div>
                </div>
        ...
        <!-- 2. Right Column: Adhérent Configuration Console -->
        <div class="col-lg-8">
            <div class="console-terminal shadow-lg">
                <div class="console-header d-flex align-items-center px-4 py-3">
                    ...
                    <span class="tiny-caps text-white fw-900 letter-spacing-sm">MODIFICATION IDENTITÉ & SÉCURITÉ CLIENT</span>
                </div>
                ...
                <form action="{{ route('client.profile.update') }}" method="POST" enctype="multipart/form-data" class="p-4 p-lg-5">
                    @csrf
                    <div class="row g-4">
                        ...
                        <div class="col-md-6">
                            <label class="label-tactical uppercase fw-700 letter-spacing-xs">Nom Complet de l'Adhérent</label>
                            <input type="text" name="name" class="form-control input-tactical uppercase fw-900" value="{{ $user->name }}" required>
                        </div>
                        
                        <div class="col-md-6">
                            <label class="label-tactical uppercase fw-700 letter-spacing-xs">Communication Digitale (Email)</label>
                            <input type="email" name="email" class="form-control input-tactical fw-900" value="{{ $user->email }}" required>
                        </div>

                        <div class="col-md-6">
                            <label class="label-tactical uppercase fw-700 letter-spacing-xs">Signal Téléphonique</label>
                            <input type="text" name="phone" class="form-control input-tactical fw-900" value="{{ $user->phone ?? '06XXXXXXXX' }}">
                        </div>

                        <div class="col-md-6">
                            <label class="label-tactical uppercase fw-700 letter-spacing-xs">Mise à jour Visuelle (Photo)</label>
                        ...
                        <div class="col-12">
                            ...
                            <input type="password" name="current_password" class="form-control input-tactical" placeholder="VÉRIFIER L'IDENTITÉ POUR MODIFIER...">
                    ...
                    <div class="text-end mt-5">
                        <button type="submit" class="btn-authorize uppercase fw-900 letter-spacing-sm">
                            <span>AUTORISER LA MISE À JOUR ADHÉRENT</span>
...
<style>
    ...
    /* Status Pips */
    .admin-crown-pip { position: absolute; bottom: 5%; right: 10%; width: 35px; height: 35px; border-radius: 50%; display: flex; align-items: center; justify-content: center; border: 2px solid var(--accent); z-index: 10; box-shadow: 0 0 15px rgba(255,237,0,0.4); }
    .bg-success-pip { background: #00ff88; color: #000; border-color: #00ff88 !important; }
    .bg-danger-pip { background: #ff3e3e; color: #fff; border-color: #ff3e3e !important; }
