...
@section('page_title', 'CENTRE DE COMMANDEMENT DU COACH')

@section('content')
<div class="container-fluid py-4 px-lg-5">
    
    <div class="row g-4 justify-content-center">
        <!-- 1. Left Column: Tactical Identity Card (Symmetrie Admin Design) -->
        <div class="col-lg-4">
                    ...
                    <div class="user-avatar-hex mx-auto mb-4" style="width: 150px; height: 150px;">
                        @if($coach->image)
                            <div class="hex-image-wrap">
                                <img src="{{ asset('storage/' . $coach->image) }}" class="hex-img" alt="Coach">
                            </div>
                        @else
                        ...
                        <div class="admin-crown-pip"><i class="fas fa-dumbbell"></i></div>
                    </div>

                    <h2 class="fw-900 text-white mb-1 uppercase letter-spacing-sm">{{ $user->name }}</h2>
                    <div class="badge-clearance mb-4 uppercase fw-900 letter-spacing-xs">NIVEAU 02 • COACH ÉLITE</div>

                    <!-- Quick Info List -->
                    <div class="tactical-data-list text-start">
                        <div class="data-item">
                            <span class="label uppercase fw-700">Identifiant Escouade</span>
                            <span class="value fw-900 text-white">#COA-{{ 200 + $coach->id }}</span>
                        </div>
                        <div class="data-item">
                            <span class="label uppercase fw-700">Spécialité</span>
                            <span class="value fw-900 text-accent">{{ strtoupper($coach->specialite) }}</span>
                        </div>
        ...
        <!-- 2. Right Column: System Configuration Console (Symmetrie Admin Design) -->
        <div class="col-lg-8">
            <div class="console-terminal shadow-lg">
                <div class="console-header d-flex align-items-center px-4 py-3">
                    ...
                    <span class="tiny-caps text-white fw-900 letter-spacing-sm">MODIFICATION IDENTITÉ & SÉCURITÉ COACH</span>
                </div>
                ...
                <form action="{{ route('coach.profile.update') }}" method="POST" enctype="multipart/form-data" class="p-4 p-lg-5">
                    @csrf
                    <div class="row g-4">
                        <div class="col-12 mb-2">
                            <h6 class="text-accent tiny-caps border-bottom border-white border-opacity-10 pb-2 fw-900">MARQUEURS PROFESSIONNELS</h6>
                        </div>
                        
                        <div class="col-md-6">
                            <label class="label-tactical uppercase fw-700 letter-spacing-xs">Nom Complet du Coach</label>
                            <input type="text" name="name" class="form-control input-tactical uppercase fw-900" value="{{ $user->name }}" required>
                        </div>
                        
                        <div class="col-md-6">
                            <label class="label-tactical uppercase fw-700 letter-spacing-xs">Spécialité de Mission</label>
                            <input type="text" name="specialite" class="form-control input-tactical uppercase fw-900" value="{{ $coach->specialite }}" required>
                        </div>

                        <div class="col-md-6">
                            <label class="label-tactical uppercase fw-700 letter-spacing-xs">Ligne de Communication (Tél)</label>
                            <input type="text" name="telephone" class="form-control input-tactical fw-900" value="{{ $coach->telephone }}" required>
                        </div>
